...
public class FinancialGetTopUpInfoApi extends BaseFinancialApi {
	...
	protected BaseResEntity action(HttpServletRequest request, HttpServletResponse response, String token)
		...
		GetTopUpInfoReqEntity reqEntity = JSON.parseObject(json, GetTopUpInfoReqEntity.class);
		reqEntity.financial_token = token;
		return FinancialDao.getTopUpInfo(reqEntity);
	}

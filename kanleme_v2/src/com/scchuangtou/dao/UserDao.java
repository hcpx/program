...
public class UserDao {
	 ...
	 * @throws SQLException
	 */
	public static int getUsersCount(DBUtils.ConnectionCache conn) throws SQLException {
		int count=0;
		PreparedStatementCache pstat = null;
		ResultSet rs = null;
		try {
			StringBuffer sb = new StringBuffer("select count(user_id) count from userinfo");
			pstat = conn.prepareStatement(sb.toString());
			rs = pstat.executeQuery();
			if (rs.next()) {
	 ...
	 * @throws SQLException
	 */
	public static int getUsersOneWeekCount(DBUtils.ConnectionCache conn) throws SQLException {
		int count=0;
		PreparedStatementCache pstat = null;
		ResultSet rs = null;
		long time=System.currentTimeMillis();
		long begin_time=time-(7*DateUtil.DAY_TIME);
		try {
			StringBuffer sb = new StringBuffer("select count(user_id) count from userinfo where login_time<=");
			sb.append(time).append("and login_time>=").append(begin_time);
			pstat = conn.prepareStatement(sb.toString());
			rs = pstat.executeQuery();
			if (rs.next()) {
				count = rs.getInt("count");
